...
const SignUp = () => {
  ...
  const signupOwnerRequest = async () => {
    const res = await axios.post("/api/user/signup/owner", {
      name,
      email,
      password,
    });
    ...
    <Box mt="80px">
      ...
      <VStack m="30px auto 50px auto" w="350px" p="15px">
        ...
        <Box border="1px solid #ebecec" p="24px">
          ...
          <Flex gap="10px">
            <Link
              className="authlinks"
              bgColor="#1877f2"
              backgroundImage='url("/icons8-facebook.svg")'
            ></Link>
            <Link
              className="authlinks"
              bgColor="#f5f5f6"
              border="1px solid #d6d8d9"
              backgroundImage='url("/icons8-google.svg")'
            ></Link>
          </Flex>
        </Box>
        ...
        <Text w="300px" fontSize="12px" mb="16px !important" textAlign="center">
          All rights reserved. Copyright 2024 - Voyago™
        </Text>
      </VStack>
    </Box>
    </>
